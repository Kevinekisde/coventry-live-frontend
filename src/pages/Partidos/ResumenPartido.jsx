...
import {
  ...
  Tabs, Badge, Button, Skeleton, Empty, Tooltip, Grid,
} from 'antd';
import {
...
function PanelEquipo({ data, esLocal, accentColorRgb, isMobile }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Zona + tipo de ataque
          En móvil van apilados, en desktop lado a lado */}
      <Row gutter={[8, 8]}>
        <Col xs={24} sm={12}>
          <Card size="small"
            title={
              <span style={{ fontSize: 12 }}>
                <AimOutlined style={{ color: '#2563eb', marginRight: 5 }} />Zona del ataque
              </span>
            }
          >
      ...
      {puntosPorJugador.length > 0 && (
        <Card size="small"
          title={
            <span style={{ fontSize: 12 }}>
              <TrophyOutlined style={{ color: accentColor, marginRight: 5 }} />Anotadores
            </span>
          }
        >
      ...
      {erroresPorJugador.length > 0 && (
        <Card size="small"
          title={
            <span style={{ fontSize: 12 }}>
              <WarningOutlined style={{ color: '#ef4444', marginRight: 5 }} />Errores por jugador
            </span>
          }
        >
...
export default function ResumenPartido() {
  ...
  const columnsSets = isMobile
    ? [
        {
          title: 'Set', dataIndex: 'numero', width: 52,
          render: (n) => <span style={{ fontWeight: 700, fontSize: 12 }}>Set {n}</span>,
        },
        {
          title: 'COV', dataIndex: 'puntosLocal',
          render: (p, s) => (
            <span style={{ fontWeight: 700, fontSize: 16, color: s.ganador === 'local' ? PINK : '#9ca3af' }}>
              {p}
            </span>
          ),
        },
        {
          title: partido.rival.slice(0, 8), dataIndex: 'puntosRival',
          render: (p, s) => (
            <span style={{ fontWeight: 700, fontSize: 16, color: s.ganador === 'rival' ? '#6366f1' : '#9ca3af' }}>
              {p}
            </span>
          ),
        },
        {
          title: '🏆', dataIndex: 'ganador', width: 36,
          render: (g) => (
            <span style={{ fontSize: 16 }}>{g === 'local' ? '🩷' : '🔵'}</span>
          ),
        },
      ]
    : [
        ...
        {
          title: 'Atlético Coventry', dataIndex: 'puntosLocal',
          render: (p, s) => (
            <span style={{ fontWeight: 700, fontSize: 18, color: s.ganador === 'local' ? PINK : '#6b7280' }}>
              {p}
            </span>
          ),
        },
        {
          title: partido.rival, dataIndex: 'puntosRival',
          render: (p, s) => (
            <span style={{ fontWeight: 700, fontSize: 18, color: s.ganador === 'rival' ? '#6366f1' : '#6b7280' }}>
              {p}
            </span>
          ),
        },
  ...
  const tabItems = [
      ...
      children: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
          ...
          <Card size="small" title="Sets jugados">
            <Table dataSource={sets} rowKey="_id" pagination={false}
              size="small" columns={columnsSets} />
          </Card>
          ...
          <Row gutter={[8, 8]}>
            <Col xs={12} sm={6}>
              <Card size="small" style={{ textAlign: 'center' }}>
                <Statistic
                  title={<span style={{ fontSize: isMobile ? 10 : 13 }}>Pts Coventry</span>}
            ...
            <Col xs={12} sm={6}>
              <Card size="small" style={{ textAlign: 'center' }}>
                <Statistic
                  title={<span style={{ fontSize: isMobile ? 10 : 13 }}>
                    Pts {isMobile ? partido.rival.slice(0, 8) : partido.rival}
                  </span>}
                  value={resumen.puntosRival}
                  valueStyle={{ color: '#6366f1', fontWeight: 700, fontSize: isMobile ? 20 : 24 }}
                />
              </Card>
            </Col>
            <Col xs={12} sm={6}>
              <Card size="small" style={{ textAlign: 'center' }}>
                <Statistic
                  title={<span style={{ fontSize: isMobile ? 10 : 13 }}>Side-out %</span>}
            ...
            <Col xs={12} sm={6}>
              <Card size="small" style={{ textAlign: 'center' }}>
                <Statistic
                  title={<span style={{ fontSize: isMobile ? 10 : 13 }}>Rallies</span>}
    ...
    {
      key: 'local',
      label: <span style={{ color: PINK, fontWeight: 600 }}>
        {isMobile ? '🩷 COV' : '🏐 Coventry'}
      </span>,
      children: (
        <div style={{ marginTop: 12 }}>
          <PanelEquipo data={local} esLocal accentColorRgb="255,79,180"
            isMobile={isMobile} />
        </div>
      ),
    },
    {
      key: 'rival',
      label: <span style={{ fontWeight: 600 }}>
        {isMobile ? `🔵 ${partido.rival.slice(0, 6)}` : `🔵 ${partido.rival}`}
      </span>,
      children: (
        <div style={{ marginTop: 12 }}>
          <PanelEquipo data={rival} esLocal={false} accentColorRgb="99,102,241"
            isMobile={isMobile} />
        </div>
      ),
    },
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4, flexWrap: 'wrap' }}>
            ...
            {partido.torneo && (
              <Tag color="blue" style={{ fontSize: 11 }}>{partido.torneo}</Tag>
            )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{
              color: PINK, fontSize: isMobile ? 9 : 11,
              fontWeight: 700, letterSpacing: isMobile ? 1 : 2, marginBottom: 6,
            }}>
              {isMobile ? 'COVENTRY' : 'ATLÉTICO COVENTRY'}
            </div>
            <div style={{
              fontSize: isMobile ? 52 : 72, fontWeight: 900,
              color: PINK, lineHeight: 1,
              textShadow: `0 0 40px ${PINK}55`,
            }}>
              {partido.setsGanados}
            </div>
            <div style={{ fontSize: isMobile ? 10 : 13, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>
              SETS
            </div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: isMobile ? 24 : 36, fontWeight: 700, color: 'rgba(255,255,255,0.2)' }}>
              –
            </div>
            <div style={{ fontSize: isMobile ? 10 : 13, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>
              {resumen.puntosLocal}–{resumen.puntosRival}
              {!isMobile && ' pts totales'}
            </div>
          </div>
